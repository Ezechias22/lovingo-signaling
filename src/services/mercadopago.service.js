...
async function mpRequest(method, path, body, idempotencyKey) {
  ...
  if (!response.ok) {
    const error = new Error(
      `Mercado Pago API error ${response.status}: ${data.message || text}`
    );
...
async function createPixPayout(payoutBody, idempotencyKey) {
  // Mercado Pago Money Out via PIX (Brazil only)
  return mpRequest('POST', '/v1/money_requests', payoutBody, idempotencyKey);
}

async function getMoneyRequest(moneyRequestId) {
  return mpRequest('GET', `/v1/money_requests/${moneyRequestId}`);
}
